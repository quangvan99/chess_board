...
class SimpleEngine:
        ...
        for i in range(10):
            for j in range(9):
                ...
                if piece and piece.team == team and piece.type == "k":
                    return (i, j)
        # Log thông báo nếu không tìm thấy vua
        print(f"King for team {team} not found on the board.")
        return None
    ...
    def king_face_each_other(self) -> bool:
        ...
        if not red_king_pos or not black_king_pos:
            # Trả về False nếu không tìm thấy cả hai vua
            return False
    ...
    def move(self, prev_board, from_pos, to_pos):
        self.load_board(prev_board)

        # Kiểm tra tính hợp lệ của bàn cờ trước khi di chuyển
        if not self.find_king("r") or not self.find_king("b"):
            print("Invalid board state: Missing one or both kings.")
            return False
