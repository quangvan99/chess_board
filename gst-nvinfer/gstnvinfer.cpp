    ...
    guint & offset_top, void *destCudaPtr)
{
  ...
  // printf("src_left %d src_top %d src_width %d src_height %d\n", src_left, src_top, src_width, src_height);
...
// void save_frame(NvBufSurface *in_surf, int i) {
//     printf("---------------\n");
//     printf("dataSize %d \n", in_surf->surfaceList[i].dataSize);
//     printf("width %d \n", in_surf->surfaceList[i].width);
//     printf("height %d \n", in_surf->surfaceList[i].height);

//     // Allocate memory for the CPU buffer
...
//     if (err != cudaSuccess) {
//         g_print("CUDA Error: %s\n", cudaGetErrorString(err));
//         free(src_data);
//         return;
//     }

//     // Handle pitch alignment
...
//     // Create a contiguous buffer for OpenCV
//     // unsigned char *contiguous_data = new unsigned char[frame_width * frame_height * 3];
//     // for (int row = 0; row < frame_height; row++) {
//     //     memcpy(&contiguous_data[row * frame_width],      // Destination (contiguous)
//     //            &((unsigned char *)src_data)[row * frame_step], // Source (pitch-aligned)
//     //            frame_width * 3);  // Width * Channels (3 for RGB)
//     // }

//     // Create OpenCV Mat from the contiguous buffer
//     cv::Mat frame_cv(frame_height, frame_width, CV_8UC3, contiguous_data);
//     cv::Mat out_mat;
//     cv::cvtColor(frame_cv, out_mat, cv::COLOR_RGB2BGR);
    ...
    NvBufSurface * in_surf)
{
  ...
  for (guint i = 0; i < num_filled; i++) {
    ...
    if (batch == nullptr) {
      ...
      // printf("batch->inbuf_batch_num %d \n", batch->inbuf_batch_num);
    ...
    // printf("source_id %d \n", frame.frame_meta->source_id);
    // printf("source_frame_width %d \n", frame.frame_meta->source_frame_width);
    // printf("source_frame_height %d \n", frame.frame_meta->source_frame_height);
    
    // printf("memory batchSize %d \n", frame.input_surf_params->batchSize);
    // printf("memory width %d \n", frame.input_surf_params->width);
    // printf("memory height %d \n", frame.input_surf_params->height);
    // printf("---------------\n");

    // // Allocate memory for the CPU buffer
    // void *src_data = malloc(frame.converted_frame_ptr->dataSize);
    // // // Copy data from GPU to CPU
    // cudaError_t err = cudaMemcpy(src_data,
    //                              frame.converted_frame_ptr->dataPtr,
    //                              frame.converted_frame_ptr->dataSize,
    //                              cudaMemcpyDeviceToHost);

    // // // Handle pitch alignment
    // size_t frame_step = frame.converted_frame_ptr->pitch;  // Pitch/stride of each row
    // gint frame_width = (gint)frame.converted_frame_ptr->width;
    // gint frame_height = (gint)frame.converted_frame_ptr->height;
    // printf("frame_width, frame_width %d %d \n", frame_width, frame_height);
    // printf("frame_step %d \n", frame_step);

    // // Create OpenCV Mat from the contiguous buffer
    // cv::Mat frame_cv(frame_height, frame_width, CV_8UC3, src_data, frame_step);
    // cv::Mat out_mat;
    // print shape of out_mat
    // cv::Size s = out_mat.size();
    // rows = out_mat.height;
    // cols = out_mat.width;
    // printf("rows %d \n", rows);
    // printf("cols %d \n", cols);
